...
export default function AdminLogin(props) {
  ...
  const login = () => {
    console.log('logging in =======>', ad_id);
    if(!admin_id && !password){
      alert('Please Provide id and password')
    }
    else{
    switch (ad_id) {
      case 'adm':
      ...
      case 'vcc':
        if (!admin_id || !password) {
          ...
          axios
            .post('http://localhost:3001/VCRegistration/login', {
              admin_id: admin_id,
              password: password,
            })
            .then(response => {
      ...
      case 'vfc':
        if (!admin_id || !password) {
          ...
          axios
            .post('http://localhost:3001/VFRegistration/login', {
              admin_id: admin_id,
              password: password,
            })
            .then(response => {
      ...
      case 'doc':
        ...
        if (!admin_id || !password) {
          ...
          axios
            .post('http://localhost:3001/doctorRegistration/login', {
              admin_id: admin_id,
              password: password,
            })
            .then(response => {
      ...
      case 'ver':
        if (!admin_id || !password) {
          ...
          axios
            .post('http://localhost:3001/verifier/login', {
              admin_id: admin_id,
              password: password,
            })
            .then(response => {
