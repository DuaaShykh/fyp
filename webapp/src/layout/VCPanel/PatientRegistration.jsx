...
export default function PatientRegistration() {
  ...
  const onSubmit = async e => {
    try {
      e.preventDefault();
      console.log(parseFloat(phone), `${f_name}${l_name}`, cnic);
      const accounts = await web3.eth.getAccounts();
      const transaction = await vaccineContract.methods
        .ProviderAddPatientRecord(parseFloat(phone), `${f_name}${l_name}`, cnic)
        .send({
          from: accounts[0],
      ...
      if (transaction) {
        ...
        data.append('martial_status', martial_status);
        ...
        data.append('zip', zip);
        data.append('cnic', cnic);
        data.append('hash', transaction.transactionHash);

        const response = await axios.post(
          'http://localhost:3001/patientRegistration',
          data,
        );
      ...
      <Form.Control type="text" name="patient_id" value={patient_id} onChange={e => onInputChange(e)} placeholder="Enter ID" />
                ...
                <Form.Group as={Col} controlId="VC_id">
                  ...
                  <Form.Control
                    ...
                    name="vc_id"
                    value={vc_id}
                    onChange={e => setVC_id(e.target.value)}
              ...
              <Form.Row>
                ...
                <Form.Group as={Col} controlId="qualification">
                  <Form.Label className="qualification">
                    Qualification
                  </Form.Label>
                  <Form.Control
                    type="text"
              ...
              <Form.Group>
                <Form.Row>
                  <Form.Label className="photo">Photo</Form.Label>
                </Form.Row>
                <input
                  type="file"
                  name="photo"
