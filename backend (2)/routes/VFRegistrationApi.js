...
router.post('/', function (req, res) {
  ...
  db.query(`INSERT INTO vf_center ( admin_id ,name , email, password , address , phone , reg_no , city ,state , zip) value("${admin_id}", "${name}",  "${email}", "${password}", "${address}", "${phone}", "${reg_no}",  "${city}", "${state}", "${zip}") `, function (error, results, fields) {
      if (error) throw error;
