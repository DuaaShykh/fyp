...
router.post('/', function (req, res) {
  ...
  if ( !vc_id && !title && !manf && !batch && !p_date && !e_date && !available) {
      return res.status(400).send({ error:true, message: 'Please provide Information to be add' });
  }

  db.query(`INSERT INTO vaccine ( vc_id , title, manf, batch , p_date , e_date , available) VALUE ("${vc_id}" , "${title}" ,  "${manf}" ,"${batch}" , "${p_date}" , "${e_date}" , "${available}" ) `, function (error, results, fields) {
      if (error) throw error;
